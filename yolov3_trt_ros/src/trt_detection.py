...
CFG = "/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/yolo3-tiny_tstl_416_depth.cfg"
TRT = '/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/model_epoch3850.trt'
NUM_CLASS = 4
...
class yolov3_trt(object):
    def __init__(self):
        ...
        if len(masks) == 2:
            ...
            self.output_shapes = [(1, output_channels, height//32, width//32), (1, output_channels,
                                                                                height//16, width//16), (1, output_channels, height//8, width//8)]
        ...
        self.detection_pub = rospy.Publisher(
            '/yolov3_trt_ros/detections', BoundingBoxes, queue_size=1)

        with open('/home/nvidia/xycar_ws/src/depth_estimation/config/config_calibration.yaml') as f:
            data = yaml.load(f, Loader=yaml.FullLoader)
            self.camera_matrix = np.array(
                data["camera_matrix"]).reshape((3, 3))
            self.dist_coeffs = np.array(data["dist_coeffs"]).reshape(
                (1, len(data["dist_coeffs"])))
            image_width = data["image_size"]["width"]
            image_height = data["image_size"]["height"]
            self.image_size = (image_width, image_height)
            self.fish_eye = data["fish_eye"]

        if (self.fish_eye == True):
            self.new_camera_matrix = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(
                self.camera_matrix, self.dist_coeffs, self.image_size, np.eye(3), 0.0)
            self.map_x, self.map_y = cv2.fisheye.initUndistortRectifyMap(
                self.camera_matrix, self.dist_coeffs, np.eye(3), self.new_camera_matrix, self.image_size, cv2.CV_32F)
        else:
            self.new_camera_matrix, _ = cv2.getOptimalNewCameraMatrix(
                self.camera_matrix, self.dist_coeffs, self.image_size, 0.0)
            self.map_x, self.map_y = cv2.initUndistortRectifyMap(self.camera_matrix, self.dist_coeffs, np.eye(
                3), self.new_camera_matrix, self.image_size, cv2.CV_32F)
        
        self.kernel_sharpening = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])

    def detect(self):
        rate = rospy.Rate(10)
        image_sub = rospy.Subscriber(
            "/usb_cam/image_raw", Imageros, img_callback)
        while not rospy.is_shutdown():
            ...
            # sharpened = cv2.filter2D(xycar_image, -1, self.kernel_sharpening)
            undist = cv2.remap(xycar_image, self.map_x,
                               self.map_y, cv2.INTER_LINEAR)

            image = self.preprocessor.process(undist)
            ...
            trt_outputs = [output.reshape(shape) for output, shape in zip(
                trt_outputs, self.output_shapes)]
            ...
            if self.show_img:
                ...
                show_img = cv2.resize(show_img, (640, 480))
                cv2.putText(show_img, "FPS:"+str(int(fps)), (10,50),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2,1)
                cv2.imshow("result",show_img)
                cv2.waitKey(1)
                #cv2.imwrite( "/home/nvidia/contest_image/result_" + str(time.time()) + ".png", show_img)
    ...
    with open(cfg, 'r') as f:
        ...
        for line in lines:
            if 'width' in line:
                ...
                anchors = [(anc[i*2], anc[i*2+1])
                           for i in range(len(anc) // 2)]
            elif 'mask' in line:
...
def img_callback(data):
    ...
    image_a = np.frombuffer(data.data, dtype=np.uint8).reshape(
        data.height, data.width, -1)
    image_b = cv2.normalize(image_a, None, 0, 255,
